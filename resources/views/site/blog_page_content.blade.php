            ...
            <div class="blog_page_content">
                ...
                @if(count($blogOne->comments))
                    <div class="article">
                        ...
                        {{--@set($com, $blogOne->comments->groupBy('parent_id'))--}}
                        @foreach($com as $key => $comments)
                            @if($key !==0)
                                @break
                            @endif
                            @include('site.blog_one_comment_tree', ['items'=>$comments])
                        @endforeach

                    </div>
                    <a class="cancel-reply-link" href="#" onclick="return removeForm() ">Cancel</a> {{-- тут невидима--}}
                @else
                    <div class="article">
                ...
                <div id="start" class="insert_new_parent_comment"></div> <!-- это для родительских комментариев -->
